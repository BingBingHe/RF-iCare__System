...
public class Container {
	...
	double phaseLast = 0;
	double bufferP[][] = new double[2][Detector.bufferSize];
	...
	public boolean canPowerStepping() {
		if (!powerFlag) {
			{
				if (rateLog.get(1).get(rateLog.get(1).size() - 1) > start.thrRDrate && this.powerBeginCounter > start.beginEnterCount)
					return true;
			}
	...
	public void setPresent(boolean present) {
		...
		if (present) {
			try {
				String path = start.path + "log.txt";

				BufferedWriter bw = new BufferedWriter(new FileWriter(path, true));
				SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//
				// 设置日期格式
				bw.write("【时间：" + df.format(new Date()) + "】" + bedNum + "开始输液");
				bw.newLine();
				bw.flush();
				bw.close();
			...
			try {
				...
				SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//
				// 设置日期格式
				bw.write("【时间：" + df.format(new Date()) + "】" + bedNum + "结束输液");
				bw.newLine();
				bw.flush();
				bw.close();
			...
			try {
				String path = start.path + System.currentTimeMillis() + ".csv";
				BufferedWriter bw = new BufferedWriter(new FileWriter(new File(path)));

				for (int j = 0; j < rateLog.get(0).size(); j++) {
					for (int i = 0; i < rateLog.size(); i++) {
						bw.write(i + ",");
						bw.write(rateLog.get(i).get(j).toString() + ",");
						bw.write(rssiLog.get(i).get(j).toString() + ",");
						bw.write(phaseLog.get(i).get(j).toString() + ",");
					}
					bw.newLine();
					bw.flush();
	...
	public double[][] getBufferP() {
		return bufferP;
	}

	public void setBufferP(double[][] bufferP) {
